...
export default function Homepage() {
	...
	useEffect(() => {
		const fetchTotalUpcomingEvents = async () => {
			...
			const { count, error } = await supabase
				...
				.gte("intFEventEndDate", currentDate.toISOString())
				.eq("intFIsHidden", 0);
	...
	useEffect(() => {
		const fetchTotalPastEvents = async () => {
			...
			const { count, error } = await supabase
				.from("internal_events")
				.select("intFID", { count: "exact" })
				.lt("intFEventEndDate", currentDate.toISOString())
				.eq("intFIsHidden", 0);
	...
	function isDateInRange(currentDate: string, startDate: string, endDate: string) {
		...
		return currentDateObj.getTime() >= startDateObj.getTime() && currentDateObj.getTime() <= endDateObj.getTime();
	}
	...
	useEffect(() => {
		const fetchGridView = async () => {
			...
			const todayEvents = mainEventData.filter(event => isDateInRange(today, event.intFEventStartDate, event.intFEventEndDate));
		...
		<div className="pl-1 pr-3 py-3 lg:p-5 space-y-4 dark:bg-dark_mode_bg bg-slate-100">
			<div className="mx-auto w-full">
				<div className="w-full flex ml-1">
					...
					<div className="md:flex bg-white border border-slate-200 rounded-lg hidden p-3 lg:p-5 gap-4 w-full dark:bg-dark_mode_card dark:border-[#27374C]">
						<div className="w-1/3 text-left h-full">
							<div className="bg-[#E5F9FF] p-5 text-slate-700 rounded-lg flex dark:bg-[#003343] dark:text-slate-300 dark:border dark:border-[#363B3D]">
								<div className="mr-4">
									<FontAwesomeIcon icon={faCalendar} className="w-8 mt-[7px] text-slate-700 dark:text-slate-300" size="2x" />
								</div>
								<div className="ml-1">
						...
						<div className="w-1/3 h-full text-left transition transform hover:scale-105">
							...
							>
								<div className="mr-[14px]">
									<FontAwesomeIcon icon={faUsers} className="w-8 mt-[7px] text-slate-700 dark:text-slate-300" size="2x" />
								</div>
								<div className="ml-1">
					...
					<div className="mt-7 bg-white border border-slate-200 rounded px-2 pt-2 pb-3 gap-2 w-full md:hidden dark:bg-dark_mode_card dark:border-[#27374C]">
						<div className="grid grid-cols-2 gap-2 h-[49px]">
							<div className="text-left transition transform hover:scale-105 h-[53px]">
								...
								>
									<div className="mr-2">
										<FontAwesomeIcon icon={faUsers} className="w-[19px] -mt-[3px] text-slate-700 dark:text-slate-300" size="2x" />
									</div>
							...
							<div className="ml-1 lg:ml-4 mb-[0px] lg:mb-[70px] dark:bg-dark_mode_card">
								...
								{eventDetails.map((detail, index) => (
									<div key={index} className="mb-7 pr-[11px]" ref={index === eventDetails.length - 1 ? lastDetailRef : null}>
										...
										<div className="flex items-center">
											<p className="text-[14px] lg:text-[17px] font-semibold text-slate-700 lg:mb-2 mt-5 dark:text-dark_text2">
												‣ Session {index + 1}
											</p>
								...
								<div className="-mt-8 lg:mt-0 lg:absolute bottom-0 left-0 right-0 p-4 bg-white flex justify-center gap-[2px] dark:bg-dark_mode_card">
									<button
										...
										className="rounded-lg px-[7px] py-[4px] lg:px-[10px] lg:py-[5px] border border-slate-800 hover:bg-slate-100 mr-4 text-[11px] lg:text-[15px] focus:shadow-outline focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900 font-medium dark:bg-dark_mode_card dark:text-slate-200 dark:border-[#7D7467] transform lg:hover:scale-105"
									>
										Add Sub-Event
									</button>
									...
									.map((subEvent, index) => (
										<div key={index}>
											...
											<div className="hidden lg:flex items-center mt-1">
												...
												<span className="mx-2 text-slate-800 ml-[15px] lg:ml-[57px] mr-6 dark:text-dark_text">|</span>
					...
					<AddSubEvent_Modal isVisible={showModalAddEvent} onClose={() => setShowModalAddEvent(false)}>
						<form>
							...
							<div className="ml-[7px] lg:ml-4 mb-[70px]">
								...
								<div className="w-full pr-[13px]">
									<p className="text-[11px] lg:text-[14px] text-mb-7 mb-[2px] font-normal text-slate-500 ml-[2px] mt-0 lg:mt-1 dark:text-dark_textbox_title">
										Event Name
										<span className="text-[12px] lg:text-[14px] text-red-500 ml-[2px] dark:text-red-600">*</span>
									</p>
									<input
										className="w-full pr-[10px] lg:pr-[11px] py-[6px] lg:py-2 pl-2 lg:pl-3 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 lg:text-sm focus:outline-none focus:border-gray-400 focus:bg-white mb-[3px] text-[12px] text-left dark:bg-dark_textbox dark:border-dark_textbox_line dark:placeholder-dark_placeholder_text dark:text-slate-300"
										type="text"
								...
								<div className="lg:absolute bottom-0 left-0 right-0 p-4 bg-white flex justify-center gap-[2px] -mb-[80px] lg:mb-0 dark:bg-dark_mode_card">
									<button className="rounded-lg px-[12px] py-[7px] lg:px-[18px] lg:py-[10px]  bg-slate-800 text-slate-100 text-[12px] lg:text-[15px] hover:bg-slate-900 focus:shadow-outline focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900 mb-2 dark:font-medium transform lg:hover:scale-105 dark:hover:bg-slate-800">
										Save Changes
									</button>
								</div>
							</div>
					...
					<EditSubEvent_Modal isVisible={showModalEditSubEvent} onClose={() => setShowModalEditSubEvent(false)}>
						<form onSubmit={handleEditSubEventSubmit}>
							...
							<div className="lg:absolute bottom-0 left-0 right-0 p-4 bg-white flex justify-center gap-[2px] -mt-[68px] lg:mt-0 dark:bg-dark_mode_card">
								<button className="rounded-lg px-[12px] py-[7px] lg:px-[18px] lg:py-[10px]  bg-slate-800 text-slate-100 text-[12px] lg:text-[15px] hover:bg-slate-900 focus:shadow-outline focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900 lg:mb-1 dark:font-medium transform lg:hover:scale-105 dark:hover:bg-slate-800">
									Save Changes
								</button>
							</div>
						</form>
					...
					<Success_CreateEventModal isVisible={showModalCreateEventSuccess} onClose={() => setShowModalCreateEventSuccess(false)}>
						<div className="p-4">
							<Image
							...
							/>
							<h3 className="text-2xl lg:text-3xl font-medium text-gray-600 mb-5 text-center -mt-8">Success!</h3>
					...
					<Success_AddSubEventModal isVisible={showModalAddEventSuccess} onClose={() => setShowModalAddEventSuccess(false)}>
						<div className="p-4">
							<Image
								src="/images/tick_mark.png"
								alt="tick_mark"
					...
					<Success_EditEventModal isVisible={showModalEditEventSuccess} onClose={() => setShowModalEditEventSuccess(false)}>
						<div className="p-4">
							<Image
								src="/images/tick_mark.png"
								alt="tick_mark"
					...
					<Success_EditSubEventModal isVisible={showModalEditSubEventSuccess} onClose={() => setShowModalEditSubEventSuccess(false)}>
						<div className="p-4">
							<Image
								src="/images/tick_mark.png"
								alt="tick_mark"
					...
					<Success_DeleteSubEventModal isVisible={showModalDeleteSubEventSuccess} onClose={() => setShowModalDeleteSubEventSuccess(false)}>
						<div className="p-4">
							<Image
								src="/images/tick_mark.png"
								alt="tick_mark"
					...
					<Delete_Event_Confirmation_Modal isVisible={showModalConfirmation} onClose={() => setShowModalConfirmation(false)}>
						<div className="p-4">
							<Image
							...
							<div className="text-center mx-auto">
								...
								<button
									...
									onClick={() => handleArchiveEvent(selectedEvent.intFID)}
								>
									Delete
								</button>
							</div>
			...
			{viewMode === 1 ? (
				<div className="w-full bg-slate-100 grid lg:grid-cols-[1fr_32%] pb-[400px] lg:pb-28 gap-4 dark:bg-dark_mode_bg">
											...
											<DropdownMenu>
												...
												<DropdownMenuContent>
													<DropdownMenuItem
														onClick={e => {
															e.stopPropagation();
															openAttendanceModal(latestEvent[0].intFID);
														}}
													>
														Attendance List
													</DropdownMenuItem>
													...
													<DropdownMenuItem
														onClick={e => {
															...
															openFeedbackModal(latestEvent[0].intFID);
														}}
													>
														Feedback Forms
													</DropdownMenuItem>
										...
										<div className="flex justify-between items-end mt-5">
											{eventsWithDaysLeft.slice(0, 1).map((event, index) => (
												<div key={index}>
													...
													>
														<span
															aria-hidden
															className="absolute inset-0 bg-red-200 opacity-50 rounded-full dark:bg-red-900"
														></span>
											...
											{eventsWithDaysLeft.every(event => event.daysLeft === 0) ? (
												<span className="relative px-3 py-[5px] font-semibold text-green-900 dark:text-green-200 text-[13px] flex items-center">
													...
													<span className="relative mt-[0px] leading-3 tracking-wider">Today</span>
												</span>
												...
												<span className="relative px-3 py-[5px] font-semibold text-orange-900 dark:text-[#BF7B5F] text-[13px] flex items-center">
													<span
														aria-hidden
														className="absolute inset-0 bg-orange-200 dark:bg-[#3F290E] opacity-50 rounded-full"
													></span>
													<AiOutlineFieldTime className="mr-1 text-[28px] font-bold relative" />
													<span className="relative mt-[0px] leading-3 tracking-wider">Tomorrow</span>
												</span>
											) : (
												<span className="relative px-3 py-[5px] font-semibold text-orange-900 dark:text-[#BF7B5F] text-[13px] flex items-center">
													<span
													...
													></span>
													<AiOutlineFieldTime className="mr-1 text-[28px] font-bold relative" />
													<span className="relative mt-[0px] leading-3 tracking-wider">Upcoming</span>
												</span>
											)}
										</div>
									</div>
											...
											<DropdownMenu>
												...
												<DropdownMenuContent>
													<DropdownMenuItem
														onClick={e => {
															e.stopPropagation();
															openAttendanceModal(latestEvent[1].intFID);
														}}
													>
														Attendance List
													</DropdownMenuItem>
													...
													<DropdownMenuItem
														onClick={e => {
															...
															openFeedbackModal(latestEvent[1].intFID);
														}}
													>
														Feedback Forms
													</DropdownMenuItem>
											...
											<DropdownMenu>
												...
												<DropdownMenuContent>
													<DropdownMenuItem
														onClick={e => {
															e.stopPropagation();
															openAttendanceModal(latestEvent[2].intFID);
														}}
													>
														Attendance List
													</DropdownMenuItem>
													...
													<DropdownMenuItem
														onClick={e => {
															...
															openFeedbackModal(latestEvent[2].intFID);
														}}
													>
														Feedback Forms
													</DropdownMenuItem>
											...
											<DropdownMenu>
												...
												<DropdownMenuContent>
													<DropdownMenuItem
														onClick={e => {
															e.stopPropagation();
															openAttendanceModal(latestEvent[3].intFID);
														}}
													>
														Attendance List
													</DropdownMenuItem>
													...
													<DropdownMenuItem
														onClick={e => {
															...
															openFeedbackModal(latestEvent[3].intFID);
														}}
													>
														Feedback Forms
													</DropdownMenuItem>
											...
											<DropdownMenu>
												...
												<DropdownMenuContent>
													<DropdownMenuItem
														onClick={e => {
															e.stopPropagation();
															openAttendanceModal(latestEvent[4].intFID);
														}}
													>
														Attendance List
													</DropdownMenuItem>
													...
													<DropdownMenuItem
														onClick={e => {
															...
															openFeedbackModal(latestEvent[4].intFID);
														}}
													>
														Feedback Forms
													</DropdownMenuItem>
											...
											<DropdownMenu>
												...
												<DropdownMenuContent>
													<DropdownMenuItem
														onClick={e => {
															e.stopPropagation();
															openAttendanceModal(latestEvent[5].intFID);
														}}
													>
														Attendance List
													</DropdownMenuItem>
													...
													<DropdownMenuItem
														onClick={e => {
															...
															openFeedbackModal(latestEvent[5].intFID);
														}}
													>
														Feedback Forms
													</DropdownMenuItem>
					...
					<div className="grid md:hidden grid-cols-1 gap-[8px] ml-1 -mr-[4.5px] -mt-2">
													...
													return (
														<div key={index}>
															...
															<div className="text-[11px] text-gray-600 mt-[5px] flex justify-between">
																...
																<span className="-mr-[6px] dark:text-dark_text">Max Attendees: {maxAttendees}</span>
															</div>
														</div>
													);
												})}
						...
						{latestEvent[4] && (
							<div
								className="bg-white border border-slate-200 rounded-lg overflow-hidden p-6 h-[340px] w-full relative flex flex-col transition transform hover:scale-105 dark:bg-dark_mode_card dark:text-slate-300 dark:border dark:border-[#363B3D] -mb-[1100px]"
								onClick={() => {
									const filteredSubEvent = subEvents.find(subEvent => subEvent.sub_eventsMainID === latestEvent[4].intFID);
				...
				>
					<div className="w-full pr-6 bg-slate-100 dark:bg-dark_mode_bg">
						<div className="w-full bg-slate-100 dark:bg-dark_mode_bg">
							<div className="ml-1 font-bold text-lg dark:text-dark_text">Today&rsquo;s Event(s)</div>
									...
									>
										<div className="ml-2 mr-2">
											...
											<div className="flex items-center mt-4">
												<HiMiniCalendarDays className="text-2xl mr-2 text-slate-800 dark:text-dark_text" />
												<p className="text-slate-600 text-sm dark:text-dark_text">
													{formatDate(event.intFEventStartDate)} - {formatDate(event.intFEventEndDate)}
												</p>
											</div>
							...
							<div className="ml-1 font-bold text-lg dark:text-dark_text">Tomorrow&rsquo;s Event(s)</div>
							<div className="border-t border-gray-300 dark:border-[#3B4043] my-4 ml-1"></div>
										...
										<div className="ml-2 mr-2">
											...
											<div className="flex justify-between items-end mt-5">
												...
												<span className="relative px-3 py-[5px] font-semibold text-yellow-900 dark:text-yellow-200 text-xs flex items-center">
													<span
														aria-hidden
														className="absolute inset-0 bg-yellow-200 dark:bg-yellow-900 opacity-50 rounded-full"
													></span>
													<AiOutlineFieldTime className="mr-1 text-2xl font-bold relative" />
							...
							<div className="ml-1 font-bold text-lg dark:text-dark_text">Upcoming Event(s)</div>
							<div className="border-t border-gray-300 dark:border-[#3B4043] my-4 ml-1"></div>
										...
										<div className="ml-2 mr-2">
											...
											<div className="flex justify-between items-end mt-5">
												...
												<span className="relative px-3 py-[5px] font-semibold text-orange-900 dark:text-red-200 text-xs flex items-center">
													<span
														aria-hidden
														className="absolute inset-0 bg-orange-200 dark:bg-red-900 opacity-50 rounded-full"
													></span>
													<AiOutlineFieldTime className="mr-1 text-2xl font-bold relative" />
