	...
	useEffect(() => {
		const fetchFacultyUnits = async () => {
			...
			if (data) {
				...
				// Extract unique categories and subcategories
				//const uniqueCategories = Array.from(new Set(data.map((unit) => unit.attsCategory)));
				//const uniqueSubcategories = Array.from(new Set(data.map((unit) => unit.attsSubcategory)));
				...
				// 			(subcategory) =>
				// 				subcategory > 0 &&
				// 				data.some((unit) => unit.attsCategory === category && unit.attsSubcategory === subcategory)
				// 		)
